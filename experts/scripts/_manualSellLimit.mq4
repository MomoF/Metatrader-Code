...
//----
      ...
      double price = WindowPriceOnDropped();
	   ...
	   {      
		...
		if( price < Bid)
		{
			Alert("Invalid price for Sell Limit!");
			return;
		}
	   ...
	   {  
   	   RefreshRates();                        // Refresh rates
   	  	ticket = OrderSend( symb, OP_SELLLIMIT, buyLots, price, 1, 0, 0, "", 0, 0, Red );//Opening Buy

   	   if(ticket > 0)                        // Success :)           
   	   {            
      	   Alert ("Opened order Sell Limit ",ticket, ", price ", price);            
      	   return;                             // Exit start()           
   	   }         
