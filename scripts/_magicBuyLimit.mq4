...
//----
      ...
      double price = WindowPriceOnDropped();
	   ...
	   {      
		...
		if( price > Ask)
		{
			Alert("Invalid price for buyStop!");
			return;
		}
	   ...
	   {  
   	   RefreshRates();                        // Refresh rates
   	  	ticket = OrderSend( symb, OP_BUYLIMIT, buyLots, price, 1, 0, 0, "", timeframe, 0, Green );//Opening Buy

   	   if(ticket > 0)                        // Success :)           
   	   {            
      	   Alert ("Opened order Buy Stop ",ticket, ", price ", price);            
      	   return;                             // Exit start()           
   	   }         
